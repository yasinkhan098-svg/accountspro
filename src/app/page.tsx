...
export default function App() {
  ...
  const handleFinalBSExport = async () => {
    if (!activeCompany) {
      alert('Pehle ek company select karein!');
      return;
    }
...
function VoucherEntryForm({activeAlterItem,activeVoucher,ledgers,stockItems,units,vouchers,activeCompany,onAltC,onSave,onDelete,onChangeType,currentDate,onF2,onPrintPreview,onCancel,voucherTypes,altCReturnContext,onAltCReturnHandled,setAltCReturnContext,onNav,setSaveToast,onSaveMaster,onOpenBankUpload}:{
  ...
  // Party side: Sales, Debit Note, Sales Quotation, Receipt, and Contra debit the party/account.
  // Purchase, Payment, and Credit Note credit the party/account.
  const partySide: 'Dr' | 'Cr' = ['Sales', 'Debit Note', 'Sales Quotation', 'Receipt', 'Contra'].includes(activeVoucher) ? 'Dr' : 'Cr';
  ...
  const [partyName, setPartyName] = useState(() => {
    ...
    if (activeVoucher === 'Payment') {
      ...
      if (crEnt && (crEnt.ledger?.name || crEnt.ledgerName)) {
        return crEnt.ledger?.name || crEnt.ledgerName;
      }
    }
    if (activeVoucher === 'Receipt' || activeVoucher === 'Contra') {
      ...
      if (drEnt && (drEnt.ledger?.name || drEnt.ledgerName)) {
        return drEnt.ledger?.name || drEnt.ledgerName;
      }
    }
  ...
  const [accEntries, setAccEntries] = useState<AccountEntry[]>(() => {
    if (!activeAlterItem || isInventory || !activeAlterItem.entries?.length) {
      const defaultType = (activeVoucher === 'Receipt' || activeVoucher === 'Contra') ? 'Cr' : 'Dr';
      return [{ ledgerId: 0, ledgerName: '', amount: 0, entryType: defaultType }];
    }

    let headerAccountName = (activeAlterItem.partyName || '').trim().toLowerCase();
    if (activeVoucher === 'Payment') {
      const crEnt = activeAlterItem.entries.find((e: any) => e.entryType === 'Cr');
      if (crEnt) headerAccountName = (crEnt.ledger?.name || crEnt.ledgerName || '').trim().toLowerCase();
    } else if (activeVoucher === 'Receipt' || activeVoucher === 'Contra') {
      const drEnt = activeAlterItem.entries.find((e: any) => e.entryType === 'Dr');
      if (drEnt) headerAccountName = (drEnt.ledger?.name || drEnt.ledgerName || '').trim().toLowerCase();
    }

    const filtered = activeAlterItem.entries
      .filter((e: any) => {
  ...
  const getList=()=>{
    if(focus?.field==='party'||focus?.field==='accledger'||focus?.field==='addl-ledger') {
      let l=ledgers.filter(l=>!filter||l.name.toLowerCase().includes(filter.toLowerCase()));
      if (focus?.field === 'accledger' && ['Payment', 'Receipt', 'Contra'].includes(activeVoucher) && partyName) {
        l = l.filter(lx => lx.name.toLowerCase() !== partyName.trim().toLowerCase());
      }
      return l;
    }
  ...
  const pickLedger=(l:Ledger)=>{
    if(focus?.field==='party'){
      ...
      if (['Payment', 'Receipt', 'Contra'].includes(activeVoucher)) {
        setAccEntries(prev => {
          const clean = prev.filter(e => (e.ledgerName || '').trim().toLowerCase() !== l.name.trim().toLowerCase());
          return clean.length > 0 ? clean : [{ ledgerId: 0, ledgerName: '', amount: 0, entryType: activeVoucher === 'Receipt' ? 'Cr' : 'Dr' }];
        });
      }
      const bal=getLedgerClosingBalance(l,vouchers);
      setPartyBalance(bal);
      ...
      if (['Payment', 'Receipt', 'Contra'].includes(activeVoucher) && partyName && l.name.trim().toLowerCase() === partyName.trim().toLowerCase()) {
        alert("The Account selected above cannot be added as a Particulars entry!");
        return;
      }
...
function BalanceSheetView({
  ...
  const getGroupTotal = (gns: string[]) =>
    gns.reduce((s, gn) => s + (grp[gn]||[]).reduce((gs,x)=>gs+x.balance,0), 0);
      ...
      <div className="report-scroll-wrapper" style={{flex:1,overflowY:'auto',padding:'16px',display:'flex',justifyContent:'center'}}>
        <div className="report-paper-card" style={{background:'#fff',width:'100%',maxWidth:1050,boxShadow:'0 2px 12px rgba(0,0,0,0.18)',fontFamily:'Arial,sans-serif'}}>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
            <tbody>
              {/* Company Header */}
              <tr><td colSpan={4} style={{padding:'8px 12px 1px 12px',fontWeight:'bold',fontSize:13,borderBottom:'none'}}>{compName}</td></tr>
              <tr><td colSpan={4} style={{padding:'1px 12px 2px 12px',fontSize:11}}>{compAddr}</td></tr>
              <tr><td colSpan={4} style={{padding:'2px 12px 8px 12px',fontWeight:'bold',fontSize:12,borderBottom:'2px solid #000'}}>BALANCE SHEET AS ON {endDate}</td></tr>

              {/* Column Headers */}
              <tr style={{background:'#f0f0f0'}}>
                <th style={{padding:'5px 8px',textAlign:'left',width:'33%',borderRight:'1px solid #aaa',borderBottom:'1px solid #aaa',fontSize:12}}>LIABILITIES</th>
                <th style={{padding:'5px 8px',textAlign:'right',width:'17%',borderRight:'2px solid #555',borderBottom:'1px solid #aaa',fontSize:12}}>AMOUNT</th>
                <th style={{padding:'5px 8px',textAlign:'left',width:'33%',borderRight:'1px solid #aaa',borderBottom:'1px solid #aaa',fontSize:12}}>ASSETS</th>
              ...
              {/* TOTAL ROW */}
              <tr style={{borderTop:'2px solid #333',borderBottom:'2px solid #333',background:'#f5f5f5'}}>
                <td style={{padding:'6px 8px',fontWeight:'bold',fontSize:12,borderRight:'1px solid #aaa'}}>TOTAL RS.</td>
                <td style={{padding:'6px 8px',textAlign:'right',fontWeight:'bold',fontSize:12,borderRight:'2px solid #555'}}>{f2(totalLiabDisplay)}</td>
                <td style={{padding:'6px 8px',fontWeight:'bold',fontSize:12,borderRight:'1px solid #aaa'}}>TOTAL RS.</td>
                <td style={{padding:'6px 8px',textAlign:'right',fontWeight:'bold',fontSize:12}}>{f2(totalAssetDisplay)}</td>
              </tr>
              ...
              {/* Signature */}
              <tr><td colSpan={4} style={{padding:'6px 12px 2px',fontSize:11}}>PLACE : {compPlace}</td></tr>
              <tr>
                <td colSpan={2} style={{padding:'4px 12px',fontSize:11,fontWeight:'bold'}}>{compName}</td>
  ...
  const closingStockValue = useMemo(()=>{
    if (stockItems.length>0) {
      ...
      for (const it of stockItems) {
        ...
        totalVal+=Math.max(0,qty)*(totalInQty>0?totalCost/totalInQty:(it.openingRate||0));
      }
      return totalVal;
    }
          ...
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
            <tbody>
              ...
              <tr style={{borderTop:'2px solid #888',borderBottom:'2px solid #888',background:'#f8f8f8'}}>
                ...
                <td style={{padding:'5px 8px',textAlign:'right',fontWeight:'bold',fontSize:12,borderRight:'2px solid #555'}}>{f2(totalTrading)}</td>
                <td style={{padding:'5px 8px',fontWeight:'bold',fontSize:12,borderRight:'1px solid #aaa'}}>TOTAL RS.</td>
              ...
              <tr style={{borderTop:'2px solid #888',borderBottom:'2px solid #888',background:'#f8f8f8'}}>
                ...
                <td style={{padding:'5px 8px',textAlign:'right',fontWeight:'bold',fontSize:12,borderRight:'2px solid #555'}}>{f2(totalPL)}</td>
                <td style={{padding:'5px 8px',borderRight:'1px solid #aaa'}}></td>
